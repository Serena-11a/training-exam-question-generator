...
- 标准格式为**题目首行 + 选项逐行 + 字段逐行式**：第一行=题型标记+题干（答案置于题干后中文圆括号内）；随后每个选项单独一行（`A.` `B.` `C.` `D.` 编号）；`解析` 单独一行、`难易度` 单独一行；**不输出分数、不输出考核点**（填空/简答的答案写入解析行），题与题之间空行分隔
- 每题含：题干、选项/答案、解析、难易度（无分数/考核点字段）
...
- **方案 A（推荐，默认）**：采用本 skill 标准题库格式 —— 每题自含（题型标记 + 题干(答案字母) + 选项逐行 + 解析 + 难易度），**题目首行 + 选项逐行式**：第一行=题型+题干（答案括号内），随后每个选项单独一行（`A.` `B.` `C.` `D.`）；`解析` 单独一行、`难易度` 单独一行；**无分数、无考核点**，到此结束。以 `references/canonical_format.md` 为唯一正确范本（详见该文件 5 类题型样例）。
...
- **严格按 `canonical_format.md` 的格式输出**：
  - 每道题自含：**第一行为**题型标记+题干(答案括号内)；**随后每个选项单独一行**（`A.` `B.` `C.` `D.` 编号）；再 `解析` 单独一行、`难易度` 单独一行；**不输出分数、不输出考核点**（填空/简答答案写入解析行）
...
## Resources
- `references/canonical_format.md` — **⭐ 权威格式规范（必读，优先级最高）**，5 类题型（题目首行+选项逐行+解析/难易度逐行，无分数/考核点）标准格式 + 出题前确认闸门
