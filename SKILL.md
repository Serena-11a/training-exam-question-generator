...
#### 确认项 3：其他参数（可给默认值，但必须明示让用户知晓）
- 总题量、各模块覆盖分布（如集团简介/企业文化/规章/绩效/报账/信息安全/行业介绍）
- 难易度与分数权重
- 是否与已有题库重复（如"���要与之前出的重合"——需先读取已有题库逐题比对）
- 输出文件格式（.txt 或考试系统导入格式）

#### ✅ 出题闸门（Gate）
只有当用户对上述**确认项 1（格式）+ 确认项 2（题型）**明确回复确认（如"确认""按这个来""可以出题"）后，才允许进入第5步。任何一项未确认，AI 须等待，严禁开工。
