...
- 标准格式为**题目首行 + 选项逐行（无前缀）+ 字段逐行式**：第一行=题型标记+题干（答案置于题干后中文圆括号内）；随后每个选项单独一行（**不带 A./B./C./D. 序号前缀**，直接写选项文字）；再 `解析` 单独一行、`难易度` 单独一行。**单选/多选/判断/简答 不输出分数、不输出考核点**（填空/简答的答案写入解析行）；**填空题例外**：在题干同行写 `填空N：答案|备选`，并额外带 `分数`、`考核点` 各一行。题与题之间空行分隔
- 每题含：题干、选项/答案、解析、难易度（单选/多选/判断/简答 无分数/考核点字段；填空含 填空N+分数+考核点）
- 若用户提供自定义模板，以其为准；否则严格沿用 `references/canonical_format.md`
- **⛔ 格式红线**：绝对禁止输出"试卷式"格式（答案速查表 + 逐题解析分离），必须每题自含（题干+选项/答案+解析+难易度 全在一起；填空题额外含 填空N+分数+考核点）
...
- **方案 A（推荐，默认）**：采用本 skill 标准题库格式 —— 每题自含：第一行=题型+题干（答案括号内），随后每个选项单独一行（**不带序号前缀**）；`解析` 单独一行、`难易度` 单独一行。**单选/多选/判断/简答 无分数、无考核点**；**填空题例外**：在题干同行写 `填空N：答案|备选`，并额外带 `分数`、`考核点` 行。以 `references/canonical_format.md` 为唯一正确范本（详见该文件 5 类题型样例）。
...
> - **解析 单独一行、难易度 单独一行**
> - **单选/多选/判断/简答 没有分数、没有考核点**；填空题额外含 分数、考核点 行
> - **每题之间有空行分隔**（不连成文字墙）
...
- **严格按 `canonical_format.md` 的格式输出**：
  - 每道题自含：**第一行为**题型标记+题干(答案括号内)；**随后每个选项单独一行（不带 A./B./C./D. 序号前缀，直接写选项文字）**；再 `解析` 单独一行、`难易度` 单独一行。**单选/多选/判断/简答不输出分数、不输出考核点**（填空/简答答案写入解析行）；**填空题例外**：在题干同行写 `填空N：答案|备选`，并额外带 `分数`、`考核点` 行
...
## Resources
- `references/canonical_format.md` — **⭐ 权威格式规范（必读，优先级最高）**，5 类题型（题目首行+选项逐行无前缀+解析/难易度逐行；填空含 填空N+分数+考核点）标准格式 + 出题前确认闸门
