...
- 标准格式为**题干单行 + 字段分行式**：题干与选项在同一行（答案置于题干后中文圆括号内，选项空格分隔同行），随后 `解析` / `难易度` / `分数` 各占一行（填空额外带 `填空N` 行与 `考核点` 行），题与题之间空行分隔
...
- **方案 A（推荐，默认）**：采用本 skill 标准题库格式 —— 每题自含（题型标记 + 题干(答案字母) + 选项 + 解析 + 难易度 + 分数），**题干单行 + 字段分行式**：题干与选项在同一行（答案置于题干后中文圆括号内，选项空格分隔同行），随后 `解析` / `难易度` / `分数` 各占一行（填空另带 填空N 行与 考核点 行）。以 `references/canonical_format.md` 为唯一正确范本（详见该文件 5 类题型样例）。
...
- **严格按 `canonical_format.md` 的格式输出**：
  - 每道题自含：**第一行为**题型标记+题干(答案括号内)+选项空格分隔同行；**随后 `解析` / `难易度` / `分数` 各占一行**（填空题另起 `填空N` 行与 `考核点` 行；简答题第二行为 `正确答案：……`）
...
## Resources
- `references/canonical_format.md` — **⭐ 权威格式规范（必读，优先级最高）**，5 类题型（题干单行+字段分行）标准格式 + 出题前确认闸门
